...
class MASFrame:
    ...
    def phewas_check(self, args) -> pl.LazyFrame:
        ...
        if args.sex_col not in args.col_names:
            logger.log(
                "IMPORTANT",
                f"sex column {args.sex_col} not found in input file. Sex-specific PheWAS filtering will not be performed.",
            )
            return self._df
        ...
        if not male_codes_in_df and not female_codes_in_df:
            logger.log(
                "IMPORTANT", "No sex-specific PheCodes found in input file. Returning all phecodes."
            )
            return self._df
        # Counts for each sex-specific code pre-filtering
        pre_counts = (
            self._df.select(sex_codes)
            .count()
            .collect()
    ...
    def check_for_constants(self, args) -> pl.LazyFrame:
        ...
        df = self._df
        const_cols = set(
            df.select(pl.col(args.selected_columns).drop_nulls().unique().len())
            .collect()
        ...
        if const_cols:
            if not args.drop_constants:
                ...
                raise ValueError(
                    f"Columns {','.join(const_cols)} are constant. Please remove them from selection or use --drop-constants."
                )
            logger.log(
                "IMPORTANT", f"Columns {','.join(const_cols)} are constant. They will be dropped."
            )
    ...
    def handle_missing_covariates(self, args) -> pl.LazyFrame:
        ...
        if new_height == 0:
            raise ValueError(
                "All rows have missing values in one of the specified covariates. Please check your data."
            )
    ...
    def validate_dependents(self, args) -> pl.LazyFrame:
        ...
        if args.quantitative:
            valid_dependents = set(
                self._df.select(args.dependents)
                .count()
                .collect()
                .transpose(include_header=True, header_name="dependent", column_names=["count"])
            ...
            if valid_dependents != args.dependents:
                logger.warning(
                    f"Dropping {len(args.dependents) - len(valid_dependents)} dependents with fewer than {args.min_cases} cases."
                )
                args.dependents = valid_dependents
            return self._df.with_columns(pl.col(args.dependents).cast(pl.Float64))

        not_binary = set(
            self._df.select(pl.col(args.dependents).unique().drop_nulls().n_unique())
            .collect()
            .transpose(include_header=True, header_name="dependent", column_names=["unique_count"])
        ...
        if not_binary:
            raise ValueError(
                f"Dependent variables {','.join(not_binary)} are not binary. Please check your data."
            )
        invalid_dependents = set(
            self._df.select(pl.col(args.dependents))
            .sum()
            .collect()
            .transpose(include_header=True, header_name="dependent", column_names=["cases"])
        ...
        if invalid_dependents:
            logger.log(
                "IMPORTANT",
                f"Dropping {len(invalid_dependents)} dependents with fewer than {args.min_cases} cases.",
            )
    ...
    def validate_flipwas(self, args) -> pl.LazyFrame:
        ...
        if not args.flipwas:
            return self._df
        not_binary = set(
            self._df.select(pl.col(args.predictors).unique().drop_nulls().n_unique())
            .collect()
            .transpose(include_header=True, header_name="predictor", column_names=["unique_count"])
            .filter(pl.col("unique_count").gt(2))
        ...
        if not_binary:
            raise ValueError(
                f"Predictors variables {','.join(not_binary)} are not binary. Please check your data."
            )
        invalid_predictors = set(
            self._df.select(pl.col(args.predictors))
            .sum()
            .collect()
            .transpose(include_header=True, header_name="predictor", column_names=["cases"])
            .filter(pl.col("cases").lt(args.min_cases))
            .get_column("predictor")
            .to_list()
        )
        if invalid_predictors:
            logger.log(
                "IMPORTANT",
                f"Dropping {len(invalid_predictors)} predictors with fewer than {args.min_cases} cases.",
            )
    ...
    def category_to_dummy(self, args) -> pl.LazyFrame:
        ...
        if not args.categorical_covariates:
            return self._df
        not_binary = set(
            self._df.select(pl.col(args.categorical_covariates).unique().drop_nulls().n_unique())
            .collect()
            .transpose(
                include_header=True, header_name="categorical_covariate", column_names=["unique_count"]
            )
            .filter(pl.col("unique_count").gt(2))
        ...
        if not_binary:
            logger.log(
                "IMPORTANT",
                f"Categorical covariates {','.join(not_binary)} are not binary. Converting to dummy variables.",
            )
            ...
            args.categorical_covariates = args.categorical_covariates.difference(not_binary).union(
                new_cols
            )
            args.independents = args.predictors.union(args.covariates)
            args.selected_columns = args.predictors.union(args.covariates).union(args.dependents)
    ...
    def run_associations(self, args) -> pl.LazyFrame:
        ...
        logger.info(
            f"Running {num_groups} associations for {len(args.predictors)} predictors and {len(args.dependents)} dependents."
        )
        ...
        if not args.flipwas:
            for predictor in args.predictors:
                ...
                for dependent in args.dependents:
                    ...
                    lazy_df = (
                        regression_df.select(pl.col(order), pl.struct(order).alias("model_struct"))
                        .drop_nulls([predictor, dependent])
                        .select(
                            pl.col("model_struct")
                            .map_batches(
                                regression_function, returns_scalar=True, return_dtype=pl.Struct
                            )
                            .alias("result")
                        )
                    )
        ...
        if args.phewas or args.flipwas:
            if args.flipwas:
                ...
                left_col = "dependent"
            result_df = result_df.join(phecode_defs, left_on=left_col, right_on="phecode").sort(
                ["predictor", "pval"]
            )
        return result_df
...
def run_multiple_association_study(args) -> pl.LazyFrame:
    ...
    result = (
        df.mas.handle_missing_covariates(args)
    ...
    if args.suffix == "predictors":
        for predictor in args.predictors:
            ...
            (result.filter(pl.col("predictor").eq(predictor)).sort("pval").write_csv(output_path))
        ...
        for dependent in args.dependents:
            ...
            (result.filter(pl.col("dependent").eq(dependent)).sort("pval").write_csv(output_path))
