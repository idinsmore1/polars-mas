...
def firth_regression_pool(
    df: pl.DataFrame, predictor: str, covariates: list[str], dependent: str, is_phewas: bool, phewas_sex_col: str|None, num_groups: int
) -> dict:
    """
    Perform Firth logistic regression on a given dataset.
    ...
    independents = [predictor, *covariates]
    regframe = df.select([predictor, *covariates, dependent]).drop_nulls([predictor, dependent])
    if regframe.height != df.height:
        logger.debug(f"Dropped {df.height - regframe.height} rows due to missing values in predictor or dependent for {dependent}.")
    reg_independents = independents.copy()
    if is_phewas:
        if dependent in sex_specific_codes and phewas_sex_col in independents:
            reg_independents = [col for col in independents if col != 'sex']
    non_consts = regframe.polars_mas.check_grouped_independents_for_constants(reg_independents, dependent)
    X = regframe.select(non_consts)
    if reg_independents[0] not in X.collect_schema().names():
        logger.warning(f"Predictor {predictor} was removed due to constant values. Skipping analysis.")
        output_struct.update(
            {
        ...
        _update_progress(num_groups)
        return output_struct
    y = regframe.get_column(dependent).to_numpy()
    cases = y.sum().astype(int)
    total_counts = y.shape[0]
    controls = total_counts - cases
    output_struct.update(
