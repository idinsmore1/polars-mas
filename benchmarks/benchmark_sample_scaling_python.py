#!/usr/bin/env python3
"""Benchmark script for polars-mas sample scaling.
...
def run_benchmark(subset_file: Path, n_samples: int, output_dir: Path) -> dict:
    """Run polars-mas with specified sample subset and return timing info."""
    covariates_str = ",".join(COVARIATES)
    output_file = output_dir / f"benchmark_sample_scaling_{n_samples}_samples_results"
...
def main():
    print("=" * 60)
    print("Python polars-mas Sample Scaling Benchmark")
    print("=" * 60)
    ...
    for n_samples in SAMPLE_SIZES:
        ...
        subset_file = output_dir / f"temp_subset_{n_samples}.csv"
    ...
    print("=" * 60)
    print(f"{'Samples':>12} {'Time (s)':>12}")
    print("-" * 26)
    for r in results:
        print(f"{r['n_samples']:>12} {r['time_seconds']:>12.2f}")

    # Save results to CSV
    output_file = output_dir / "benchmark_sample_scaling_python_results.csv"
    with open(output_file, "w") as f:
        f.write("n_samples,time_seconds\n")
        for r in results:
            f.write(f"{r['n_samples']},{r['time_seconds']:.4f}\n")
